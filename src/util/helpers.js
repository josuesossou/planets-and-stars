 ...
 */
export const createSpaceObject = (radius, mass, color, name, canvas, type, scaleFactor, ignoreDistance) => {
    return new SpaceObject(radius, mass, color, name, canvas, type, scaleFactor, ignoreDistance)
}

/**
 * Calculates the distance between two space objects. distance = sqrt((x2-x1)^2 + (y2-y1)^2 + (z2-z1)^2)
 * distance coordinates is in AU
 ...
 * 
 * coordinates are in AU. in this program, 1 AU = 200 pixels
 */
export const calculateDistance = (coords1, coords2) => {
    ...
    return Math.sqrt(distanceSquared) // distance in AU
}

/**
 * Calculates the force of gravity on an object using Force = GM1M2/d^2 formula
 * mass is the ratio of a planet mass to the mass of earth
 * we know that the gravity on earh is 9.8
